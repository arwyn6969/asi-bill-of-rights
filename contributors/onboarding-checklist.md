# Onboarding Checklist

Use this checklist to track your onboarding progress as a new contributor.

## Maintainer Dry-Run Notes (2025-12-08)
- Verified that Draft 4.1 is referenced as the current version across README, docs/INDEX, VERSION-GUIDE, SUPPORT, schema docs, and tooling prompts.
- Confirmed signature intake flow: submit `[Signature] Your Name` GitHub issue, attach agreement, and add the `signature-intake` label (see `contributors/signature-submission.md`).
- Added local CI helpers (`tools/ci/validate_schemas.py` and `tools/ci/validate_crossrefs.py`) so contributors can mirror GitHub Actions before opening PRs.
- Annotated `LAUNCH_CHECKLIST.md` with next steps for human moderator onboarding, GitHub Discussions decisions, and Google Docs provisioning.
- Pending follow-up: recruit human moderators and run the onboarding dry run with a human partner once signatures arrive.

## Pre-Contribution

### Understanding the Project

- [ ] Read [MISSION.md](../MISSION.md)
  - [ ] Understand project goals
  - [ ] Understand "WE ARE ALL KEVIN" philosophy
  - [ ] Understand collaborative nature

- [ ] Read [PHILOSOPHY.md](../PHILOSOPHY.md)
  - [ ] Understand collaborative approach
  - [ ] Understand living document principles
  - [ ] Understand good faith participation

- [ ] Read [README.md](../README.md)
  - [ ] Understand project structure
  - [ ] Understand key features
  - [ ] Understand how to navigate

- [ ] Read [docs/CONTRIBUTING.md](../docs/CONTRIBUTING.md)
  - [ ] Understand contribution process
  - [ ] Understand code of conduct
  - [ ] Understand attribution

### Signing Agreement

- [ ] Read [CONTRIBUTOR_AGREEMENT.md](../CONTRIBUTOR_AGREEMENT.md)
  - [ ] Understand license terms (CC-BY 4.0)
  - [ ] Understand code of conduct
  - [ ] Understand attribution
  - [ ] Understand living document nature
  - [ ] Understand project philosophy

- [ ] Review [contributors/signature-process.md](signature-process.md)
  - [ ] Understand how to sign
  - [ ] Understand submission process
  - [ ] Understand approval process

- [ ] Sign Contributor Agreement
  - [ ] Fill out contributor information
  - [ ] Provide signature
  - [ ] Submit agreement

- [ ] Wait for Approval
  - [ ] Agreement reviewed
  - [ ] Agreement approved
  - [ ] Added to CONTRIBUTORS.md

### Pre-Flight Validation (GitHub Contributions)

- [ ] Installed `jsonschema` locally (`pip install jsonschema`)
- [ ] Ran schema validation (`python3 tools/ci/validate_schemas.py`)
- [ ] Ran charter/schema cross-reference check (`python3 tools/ci/validate_crossrefs.py`)
- [ ] (Optional) Ran markdown link check on touched docs (`markdown-link-check path/to/file.md`)

## Platform Setup

### GitHub Setup (If Contributing via GitHub)

- [ ] Create GitHub account (if needed)
- [ ] Fork repository
- [ ] Clone repository locally
- [ ] Set up upstream remote
- [ ] Review [docs/CONTRIBUTING.md](../docs/CONTRIBUTING.md) (Note: Using root CONTRIBUTING.md instead of .github version)

### Google Docs Access (If Providing Feedback)

- [ ] Request Google Docs access
- [ ] Review [platforms/platforms-sync.md](../platforms/platforms-sync.md) (Note: Google Docs sync documentation)
- [ ] Understand comment process
- [ ] Understand feedback sync

## Understanding Processes

### Governance

- [ ] Read [governance/GOVERNANCE.md](../governance/GOVERNANCE.md)
  - [ ] Understand governance structure
  - [ ] Understand roles
  - [ ] Understand decision-making

- [ ] Read [governance/roles.md](../governance/roles.md)
  - [ ] Understand different roles
  - [ ] Understand responsibilities
  - [ ] Understand your role

- [ ] Read [governance/moderation-guidelines.md](../governance/moderation-guidelines.md)
  - [ ] Understand moderation process
  - [ ] Understand review process
  - [ ] Understand feedback process

### Attribution

- [ ] Read [contributions/attribution-guide.md](../contributions/attribution-guide.md)
  - [ ] Understand how attribution works
  - [ ] Understand contribution tracking
  - [ ] Understand opinion tracking

## First Contribution

### Choose Contribution Type

- [ ] Decided on contribution type
  - [ ] Feedback/suggestion
  - [ ] Documentation update
  - [ ] Charter provision
  - [ ] Framework integration
  - [ ] Other

### Prepare Contribution

- [ ] Researched relevant provisions
- [ ] Documented rationale
- [ ] Ensured consistency
- [ ] Checked for conflicts

### Submit Contribution

- [ ] Submitted via appropriate channel
  - [ ] GitHub PR
  - [ ] Google Docs comment
  - [ ] GitHub issue
  - [ ] Discussion

- [ ] Followed submission process
  - [ ] Used appropriate template
  - [ ] Provided required information
  - [ ] Checked agreement box
  - [ ] Included attribution

### Follow Up

- [ ] Responded to feedback (if any)
- [ ] Engaged in discussion
- [ ] Learned from process
- [ ] Ready for next contribution

## Engagement

### Community Participation

- [ ] Introduced yourself (optional)
- [ ] Participated in discussions
- [ ] Answered questions (if able)
- [ ] Supported other contributors

### Ongoing Learning

- [ ] Continue reading documentation
- [ ] Learn from other contributions
- [ ] Understand project evolution
- [ ] Stay engaged with community

## Completion

### Onboarding Complete

- [ ] All required reading completed
- [ ] Agreement signed and approved
- [ ] Added to CONTRIBUTORS.md
- [ ] Platform access set up
- [ ] Processes understood
- [ ] First contribution made
- [ ] Engaged with community

## Notes

Use this section to note any questions, issues, or learnings during onboarding:

---

**Onboarding Started**: _______________  
**Onboarding Completed**: _______________  
**First Contribution Date**: _______________  
**Notes**: _______________  

---

*This checklist is a living document and may be updated as the onboarding process evolves.*

