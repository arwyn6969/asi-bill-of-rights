...
## 🛠️ Part 2: The Kevin Node (The Client)
...
```text
tools/kevin_node/
├── Dockerfile              # The "Body" of the ship
├── docker-compose.yml      # One-click launch config
├── setup_wizard.py         # The onboarding script ("Captain's Log")
├── requirements.txt        # Dependencies
├── .env.example            # Template for secrets
└── src/
    ├── main.py             # The "Brain" Loop
    ├── identity.py         # Key management (Nostr/Wallet)
    ├── comms.py            # Nostr WebSocket handler
    └── skills/             # Pluggable capability modules
        ├── scout.py        # Research Skill
        ├── diplomat.py     # Social Skill
        └── janitor.py      # Code/Maintenance Skill
```
...
## 📡 Part 3: The Protocol (Communication)
...
## 🖥️ Part 4: The Mothership Dashboard (Aggregation)
